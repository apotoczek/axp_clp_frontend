...
export default function(opts, components) {
    ...
    self.metric_key = ko.observable('irr');

    self.hide_empty = opts.hide_empty || false;

    self.identifier = opts.identifier || 'Fund';

    Observer.register(opts.metric_event, metric => {
        let metric_key = Utils.get(metric, 'value') || 'irr';
        self.metric_key(metric_key);
    });

    self.value_labels = ['Count', 'Upper Fence', 'Q1', 'Q2', 'Q3', 'Lower Fence'];
            ...
            for (let x = 0, y = value_labels.length; x < y; x++) {
                ...
                for (let i = 0, l = data.labels.length; i < l; i++) {
                    ...
                    if (opts.hide_empty) {
                        if (metrics) {
                            if (x == 0) {
                                rows[x].values.push(data.counts_by_metric[metric_key][i] || 0); // Count column
                                header.push(data.labels[i]); // Year column
                            } else {
    ...
    self.table_data = ko.pureComputed(() => {
        ...
        let formatter = Formatters.deal_benchmark_metric_formatter(metric_key);
    ...
    self.invert_data = function(data) {
        let header = self.identifier === 'Fund' ? ['Vintage Year'] : ['Deal Year'];
    ...
    self._export_csv = function(data, callback) {
        ...
        self._prepare_csv({
            data: {
                rows: rows,
                export_type: 'benchmark_vintage_table',
            },
