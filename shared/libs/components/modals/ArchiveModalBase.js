...
export default function(opts, components) {
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <!-- ko renderComponent: to_archive --><!-- /ko -->
                            <hr class="transparent hr-small">
                            <p data-bind="text: warning_text"></p>
                            <hr class="transparent hr-small">
                            <button type="button" class="btn btn-danger" data-bind="click: archive_entities, css: { disabled: loading }">
                                <!-- ko if: loading -->
    ...
    self.warning_text = 'Are you sure you want to archive these entities?';
    self.btn_text = 'Archive';

    /********************************************************************
     * Table of stuff to be archived
     *******************************************************************/

    self.data_to_archive = ko.computed(() => {
        let data = self.data();
        if (data) {
    ...
    self.to_archive = new DataTable({
        parent_id: self.get_id(),
        id: 'to_archive',
        results_per_page: 10,
        inline_data: true,
        css: 'table-light table-sm',
        data: self.data_to_archive,
        columns: opts.to_archive_table_columns || [
            {
                label: 'Name',
                key: 'name',
            },
    ...
    self.archive_entities = function() {
        throw 'Archive entities has to be implemented in subinstance of ArchiveModalBase..';
    };
