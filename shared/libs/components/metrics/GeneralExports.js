...
export default function(opts, components) {
    ...
    let by_client_chart = Object.assign(MetricsHelper.get_barchart_config({}), {
        id: 'by_client',
        value_key: 'count',
    ...
    let by_period_chart = Object.assign(MetricsHelper.get_timeseries_config('Reports Run'), {
        id: 'by_period',
        x_formatter: time => MetricsHelper.period_formatter(time, self.active_group.peek()),
        x_units: () => (self.active_group.peek() == 'week' ? [['week', [1, 2, 3, 4]]] : undefined),
        zoom_event: self.page_events.get('chart_selection'),
        datasource: MetricsHelper.get_datasource({
            target: 'commander:exports',
            period_event: self.page_events.get('time_period_changed'),
    ...
    let chart_wrapper = {
        ...
        layout: {
            body: ['default', 'by_client', 'by_type', 'by_period'],
        },
        components: [chart, by_client_chart, by_type_chart, by_period_chart],
    };
    ...
    let by_client_table = Object.assign(MetricsHelper.get_datatable_config({}), {
        id: 'by_client',
        label: 'Exports grouped by client',
        columns: MetricsHelper.get_columns_from_keys(['client_name', 'export_count']),
        datasource: MetricsHelper.get_datasource({
            target: 'commander:exports',
            period_event: self.page_events.get('time_period_changed'),
            group_by: 'client',
    ...
    let table_wrapper = {
        ...
        components: [table, by_client_table, by_type_table, by_period_table],
    };
