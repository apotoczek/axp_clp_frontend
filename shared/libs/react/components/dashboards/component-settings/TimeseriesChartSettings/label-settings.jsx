...
const INTERVAL_CATEGORIZATION_DESCRIPTION = [
    {
        description: oneLine`
        Used to group the time across the x axis into groups. This can be used to create charts
        that have discrete time steps across the axis instead of a continuous time axis.
        For example an axis could be split into 4 groups, one for each quarter of a year.
    `,
    },
];
...
export function LabelSettings({
    ...
    return (
        ...
        >
            ...
            {axisName === 'xAxis' && (
                <InfoTextDropdown
                    title='Categorize Intervals'
                    infoTexts={INTERVAL_CATEGORIZATION_DESCRIPTION}
                >
                    <Checkbox
                        leftLabel='Categorize Intervals'
                        checked={provider.settingsValueForComponent([
                            `${axisName}Label`,
                            'categorizeIntervals',
                        ])}
                        onValueChanged={value =>
                            onLabelSettingChanged({key: 'categorizeIntervals', value})
                        }
                    />
                </InfoTextDropdown>
            )}
            ...
            <NumberInput
                ...
                value={provider.settingsValueForComponent([`${axisName}Label`, 'tickRotation'])}
                ...
                <>
                    <Flex mb={1}>
                        <DropdownList
                            ...
                            onValueChanged={value =>
                                onLabelSettingChanged({key: 'displayUnits', value})
                            }
                            mr={1}
                        />
                        <Checkbox
                            ...
                            onValueChanged={value =>
                                onLabelSettingChanged({key: 'showUnit', value})
                            }
                        />
                    </Flex>
                    <NumberInput
                        ...
                        onValueChanged={value =>
                            onLabelSettingChanged({key: 'decimalPlaces', value})
                        }
                        mb={1}
                    />
                    {axisFormat === Format.Money && (
                        <TextInput
                            ...
                            onValueChanged={value =>
                                onLabelSettingChanged({key: 'currencySymbol', value})
                            }
                            mb={1}
                        />
                    )}
        ...
        <Collapsible header={header} isOpen={isOpen} toggleOpen={() => setIsOpen(!isOpen)}>
            ...
            >
                <TextInput
                    ...
                    placeholder='E.g. {{value}} ({{valueName}})'
